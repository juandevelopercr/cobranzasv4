window.renderFusionCharts = function (chartData) {
  ...
  const pieFormalizacionMesUSDContainerId = 'formalizaciones_bank_mes_usd';
  ...
  if (pieformalizacionMesUSDContainer) {
    const validData = chartData.pie_formalizaciones_bank_mes_usd?.data
      ?.map(item => ({
        ...item,
    ...
    if (!hasDataFormalizacionMesUSDPie) {
        ...
        <div class="no-data-container">
            <div class="no-data-content">
              ...
              <p class="no-data-message">
                ${chartData.pie_formalizaciones_bank_mes_usd?.caption || ''}
                ${chartData.pie_formalizaciones_bank_mes_usd?.subCaption ? '| ' + chartData.pie_formalizaciones_bank_mes_usd.subCaption : ''}
              </p>
      ...
      try {
        const chartConfig = {
          ...
          dataSource: {
            chart: {
              caption: chartData.pie_formalizaciones_bank_mes_usd.caption || 'Formalizaciones Bancos en USD',
              subCaption: chartData.pie_formalizaciones_bank_mes_usd.subCaption || '',
  ...
  const pieFormalizacionMesCRCContainerId = 'formalizaciones_bank_mes_crc';
  ...
  if (pieformalizacionMesCRCContainer) {
    const validData = chartData.pie_formalizaciones_bank_mes_crc?.data
      ?.map(item => ({
        ...item,
        value: parseFloat(item.value),
        numericValue: parseFloat(item.value)
    ...
    if (!hasDataFormalizacionMesCRCPie) {
      pieformalizacionMesCRCContainer.innerHTML = `
        <div class="no-data-container">
            <div class="no-data-content">
              ...
              <h3 class="no-data-title">No hay datos disponibles</h3>
              <p class="no-data-message">
                ${chartData.pie_formalizaciones_bank_mes_crc?.caption || ''}
                ${chartData.pie_formalizaciones_bank_mes_crc?.subCaption ? '| ' + chartData.pie_formalizaciones_bank_mes_crc.subCaption : ''}
              </p>
              <p class="no-data-message">Intenta con otros filtros o parámetros</p>
            </div>
          </div>
      ...
      try {
        const chartConfig = {
          ...
          dataSource: {
            chart: {
              caption: chartData.pie_formalizaciones_bank_mes_crc.caption || 'Formalizaciones Bancos en CRC',
              subCaption: chartData.pie_formalizaciones_bank_mes_crc.subCaption || '',
              theme: chartData.theme || 'zune',
  ...
  const pieFormalizacionYearUSDContainerId = 'formalizaciones_bank_year_usd';
  ...
  if (pieformalizacionYearUSDContainer) {
    const validData = chartData.pie_formalizaciones_bank_year_usd?.data
      ?.map(item => ({
        ...item,
        value: parseFloat(item.value),
        numericValue: parseFloat(item.value)
    ...
    if (!hasDataFormalizacionYearUSDPie) {
      pieformalizacionYearUSDContainer.innerHTML = `
        <div class="no-data-container">
            <div class="no-data-content">
              ...
              <h3 class="no-data-title">No hay datos disponibles</h3>
              <p class="no-data-message">
                ${chartData.pie_formalizaciones_bank_year_usd?.caption || ''}
                ${chartData.pie_formalizaciones_bank_year_usd?.subCaption ? '| ' + chartData.pie_formalizaciones_bank_year_usd.subCaption : ''}
              </p>
              <p class="no-data-message">Intenta con otros filtros o parámetros</p>
            </div>
          </div>
      ...
      try {
        const chartConfig = {
          ...
          dataSource: {
            chart: {
              caption: chartData.pie_formalizaciones_bank_year_usd.caption || 'Formalizaciones Bancos en USD',
              subCaption: chartData.pie_formalizaciones_bank_year_usd.subCaption || '',
              theme: chartData.theme || 'zune',
              paletteColors: getRotatedColors(chartData.theme || 'zune', 0).join(','),
              numberPrefix: '',
              formatNumberScale: '0',
  ...
  const pieFormalizacionYearCRCContainerId = 'formalizaciones_bank_year_crc';
  ...
  if (pieformalizacionYearCRCContainer) {
    const validData = chartData.pie_formalizaciones_bank_year_crc?.data
      ?.map(item => ({
        ...item,
        value: parseFloat(item.value),
        numericValue: parseFloat(item.value)
    ...
    if (!hasDataFormalizacionYearCRCPie) {
      pieformalizacionYearCRCContainer.innerHTML = `
        <div class="no-data-container">
            <div class="no-data-content">
              ...
              <h3 class="no-data-title">No hay datos disponibles</h3>
              <p class="no-data-message">
                ${chartData.pie_formalizaciones_bank_year_crc?.caption || ''}
                ${chartData.pie_formalizaciones_bank_year_crc?.subCaption ? '| ' + chartData.pie_formalizaciones_bank_year_crc.subCaption : ''}
              </p>
              <p class="no-data-message">Intenta con otros filtros o parámetros</p>
            </div>
          </div>
      ...
      try {
        const chartConfig = {
          ...
          dataSource: {
            chart: {
              caption: chartData.pie_formalizaciones_bank_year_crc.caption || 'Formalizaciones Bancos en CRC',
              subCaption: chartData.pie_formalizaciones_bank_year_crc.subCaption || '',
              theme: chartData.theme || 'zune',
  ...
  //*******************************************************************************************************//
  //********************************** Gráfico de Heatmap Monedas******************************************//
  //*******************************************************************************************************//

  const heatmapCurrencyContainerId = 'formalizaciones_moneda_heatmap';
  const heatmapCurrencyContainer = document.getElementById(heatmapCurrencyContainerId);

  if (heatmapCurrencyContainer) {
    // Verificar si hay datos para mostrar
    const hasDataHeatmapCurrency = chartData.heatmapCurrency?.dataset?.some(d =>
      d.data?.some(v => v.value !== 0 && v.value !== null)
    );

    if (!hasDataHeatmapCurrency) {
      heatmapCurrencyContainer.innerHTML = `
        <div class="no-data-container">
            <div class="no-data-content">
              ...
              <h3 class="no-data-title">No hay datos disponibles</h3>
              <p class="no-data-message">
                ${chartData.heatmapCurrency?.caption || ''}
                ${chartData.heatmapCurrency?.subCaption ? '| ' + chartData.heatmapCurrency.subCaption : ''}
              </p>
              <p class="no-data-message">Intenta con otros filtros o parámetros</p>
            </div>
          </div>
      ...
      if (window.fusionChartInstances[heatmapCurrencyContainerId]) {
        window.fusionChartInstances[heatmapCurrencyContainerId].dispose();
        delete window.fusionChartInstances[heatmapCurrencyContainerId];
      }
    } else {
      if (window.fusionChartInstances[heatmapCurrencyContainerId]) {
        window.fusionChartInstances[heatmapCurrencyContainerId].dispose();
      ...
      try {
        ...
        const chartConfig = {
          type: 'heatmap',
          renderAt: heatmapCurrencyContainerId,
          width: '100%',
          height: '300', // Aumentamos la altura para mejor visualización
          dataFormat: 'json',
          dataSource: {
            chart: {
              caption: chartData.heatmapCurrency.caption || '-',
              subCaption: chartData.heatmapCurrency.subCaption || '',
              theme: chartData.theme || 'zune',
              valueFontSize: '12',
              showLabels: '1',
              showValues: '1',
            ...
            rows: chartData.heatmapCurrency.rows || { row: [] },
            columns: chartData.heatmapCurrency.columns || { column: [] },
            dataset: chartData.heatmapCurrency.dataset || [],
            colorrange: chartData.heatmapCurrency.colorrange || {
              gradient: '1',
              minvalue: '0',
              code: '#FCFBFF',
              color: [
        ...
        window.fusionChartInstances[heatmapCurrencyContainerId] = new FusionCharts(chartConfig);
