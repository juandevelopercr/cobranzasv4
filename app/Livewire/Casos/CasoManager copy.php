...
class CasoScotiabank extends BaseComponent
{
  ...
  private array $varchar10Fields = ['nmarchamo', 'pestado_arreglo', 'codigo_activacion', 'tiempo_dias', 'tiempo_annos'];

  private array $varchar30Fields = ['ptelefono_demandado_deudor_o_arrendatario', 'pplaca1', 'pplaca2', 'pnumero_cedula_juridica'];

  private array $varchar50Fields = [
    'pcedula_deudor',
    'pcedula_arrendatario',
    'pcorreo_demandado_deudor_o_arrendatario',
    'dnumero_carnet',
  ...
  private array $varchar100Fields = [
    'pnombre_apellidos_deudor',
    'pnombre_arrendatario',
    'pnombre_contacto_o_arrendatario',
    'pnombre_coarrendatario',
  ...
  private array $varchar150Fields = ['nombre_cliente', 'empresa'];

  private array $varchar160Fields = ['email_cliente'];

  private array $varchar200Fields = [
    'psaldo_dolarizado',
    'pnumero_operacion1',
    'pnombre_demandado',
    'pnumero_expediente_judicial',
  ...
  private array $varchar255Fields = ['pdespacho_judicial_juzgado', 'pdatos_codeudor2'];
  ...
  private array $intFields = ['pnumero', 'pexpectativa_recuperacion_id'];

  private array $bigintFKFields = [
  ...
  {
    return CasoEstado::orderBy('name', 'ASC')->get();
  }

  public function mount()
  {
    // Banco Scotiabank costa rica
    $this->bank_id = Bank::SCOTIABANKCR;
    // Catálogos básicos
    $this->currencies = Currency::orderBy('code')->get();
    $this->banks = Bank::where('name', 'like', '%Scotia%')->get();

    $this->abogados = User::where('active', 1)
      ->whereHas('roles', fn($q) => $q->where('name', User::ABOGADO))
      ->orderBy('name')->get();
  ...
  public function render()
  {
    ...
    // Solo casos del banco Scotiabank si se encontró el ID
    if ($this->bank_id) {
      $query->where('casos.bank_id', $this->bank_id);
    }
    ...
    return view('livewire.casos.scotiabank-datatable', [
      'records' => $records,
    ]);
  }
  ...
  public function rules()
  {
    ...
    // Bigint FKs
    foreach ($this->bigintFKFields as $field => $table) {
      $rules[$field] = 'nullable|exists:' . $table;
    }
    // Obligatorio para el banco
    ...
    // DECIMAL
    foreach ($this->decimalFields as $f) {
      $rules[$f] = 'nullable|numeric';
    }

    // DATE / TIMESTAMP
    foreach ($this->dateFields as $f) {
      $rules[$f] = 'nullable|date';
    }

    // VARCHAR(2)
    foreach ($this->varchar2Fields as $f) $rules[$f] = 'nullable|string|max:2';
    // VARCHAR(10)
    foreach ($this->varchar10Fields as $f) $rules[$f] = 'nullable|string|max:10';
    // VARCHAR(30)
    foreach ($this->varchar30Fields as $f) $rules[$f] = 'nullable|string|max:30';
    // VARCHAR(50)
    foreach ($this->varchar50Fields as $f) $rules[$f] = 'nullable|string|max:50';
    // VARCHAR(100)
    foreach ($this->varchar100Fields as $f) $rules[$f] = 'nullable|string|max:100';
    // VARCHAR(150)
    foreach ($this->varchar150Fields as $f) $rules[$f] = 'nullable|string|max:150';
    // VARCHAR(160)
    foreach ($this->varchar160Fields as $f) $rules[$f] = 'nullable|string|max:160';
    // VARCHAR(200)
    foreach ($this->varchar200Fields as $f) $rules[$f] = 'nullable|string|max:200';
    // VARCHAR(255)
    foreach ($this->varchar255Fields as $f) $rules[$f] = 'nullable|string|max:255';

    // TEXT
    foreach ($this->textFields as $f) $rules[$f] = 'nullable|string';

    // Campos no clasificados arriba (por si acaso)
    $ensure = [
      'pnumero_expediente_judicial',
      'pnumero_contrato',
      'pnumero_operacion2',
    ...
    foreach ($ensure as $f) {
      if (!isset($rules[$f])) $rules[$f] = 'nullable|string|max:255';
    }
  ...
  {
    $data = $this->collectAllFieldsForPersistence();
  ...
  public function edit($id)
  {
    ...
    foreach ($this->fieldList() as $f) {
      if (in_array($f, $this->dateFields) && !empty($record->{$f})) {
        $this->{$f} = Carbon::parse($record->{$f})->format('d-m-Y');
      } else {
        $this->{$f} = $record->{$f};
      }
    }
  ...
  {
    // Mantiene compatibilidad con tu estructura; puedes conectar a DataTableConfig si lo usas aquí también.
