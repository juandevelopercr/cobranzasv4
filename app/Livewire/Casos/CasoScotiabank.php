...
class CasoScotiabank extends CasoManager
{
  ...
  public function refresDatatable()
  {
    $config = DataTableConfig::where('user_id', Auth::id())
      ->where('datatable_name', 'casos-scotiabanks-datatable')
      ->first();
      ...
      for ($r = 1; $r < count($sheet); $r++) {
          ...
          if ($pnumero) {
              $caso = \App\Models\Caso::where([
                  'pnumero' => $pnumero,
                  'bank_id' => Bank::SCOTIABANKCR
              ])->first();
          }
          if (!$caso) {
              $caso = new \App\Models\Caso();
              $caso->bank_id = Bank::SCOTIABANKCR;
  ...
  */
  public function importar()
  {
    ...
    $columns = ImportColumns::getColumnasPorBanco(Bank::SCOTIABANKCR);
    ...
    for ($r = 1; $r < count($sheet); $r++) {
        ...
        if ($pnumero) {
            ...
            if ($caso && $caso->bank_id != Bank::SCOTIABANKCR){
