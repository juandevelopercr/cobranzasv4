...
class ComisionistaManager extends BaseComponent
{
  use WithFileUploads;
  use WithPagination;
  ...
  #[Url(history: true)]
  public $sortBy = 'comisionistas.id';
  ...
  {
    $records = Comisionista::search($this->search, $this->filters) // Utiliza el scopeSearch para la búsqueda
      ->orderBy($this->sortBy, $this->sortDir)
      ->paginate($this->perPage);

    return view('livewire.clasificadores.comisionistas.datatable', [
      'records' => $records,
    ]);
  }
  ...
  {
    $rules = [
      'corr'  => 'required|string|max:10|unique:comisionistas,corr,' . $this->recordId . ',id',
      'nombre'  => 'required|string|max:150',
      'impresion'  => 'nullable|string|max:10',
  ...
  {
    return [
      'corr.required' => 'El código es obligatorio.',
      'corr.unique' => 'Este código ya está registrado.',
      'corr.max' => 'El código no debe superar los 20 caracteres.',

      'nombre.required' => 'El nombre es obligatoria.',
      'nombre.max' => 'El nombre no debe superar los 150 caracteres.',

      'impresion.max' => 'El nombre corto no debe superar los 10 caracteres.',
    ];
  }
  ...
  public function store()
  {
    ...
    try {
      ...
      $record = Comisionista::create($validatedData);
  ...
  public function edit($recordId)
  {
    ...
    $record = Comisionista::find($recordId);
    $this->recordId = $recordId;

    $this->corr = $record->corr;
    $this->nombre = $record->nombre;
    $this->impresion = $record->impresion;
  ...
  public function update()
  {
    ...
    try {
      ...
      $record = Comisionista::findOrFail($recordId);
  ...
  public function refresDatatable()
  {
    $config = DataTableConfig::where('user_id', Auth::id())
      ->where('datatable_name', 'classifier-comisionista-datatable')
      ->first();
