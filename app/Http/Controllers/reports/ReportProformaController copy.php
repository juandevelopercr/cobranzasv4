...
class ReportProformaController extends Controller
{
  ...
  public function prepararExportacionReciboSencillo($key)
  {
    try {
      $params = Cache::pull($key);
      ...
      $filename = \App\Helpers\Helpers::generateReciboSencilloPdf($reciboId);

      return response()->json([
        'filename' => $filename
      ]);
    } catch (Throwable $e) {
      Log::error("Error al preparar exportación de recibo sencillo: " . $e->getMessage());
      return response()->json(['error' => 'Error al generar el archivo'], 500);
    }
  }

  public function descargarExportacionReciboSencillo($filename)
  {
    $path = storage_path("app/public/recibos_sencillos/$filename");
