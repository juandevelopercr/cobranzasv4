<?php

namespace App\Http\Controllers\reports;

use App\Models\Contact;
use Illuminate\Http\Request;
use App\Exports\GeneralReport;
use App\Http\Controllers\Controller;
use Illuminate\Support\Facades\Cache;
use Maatwebsite\Excel\Facades\Excel;

class ReportGeneralesController extends Controller
{
  public function index()
  {
    //$clients = Contact::all();
    return view('content.reports.generales');
  }

  public function downloadGenerales(string $key)
  {
    $filters = Cache::pull($key);
    if (!is_array($filters)) {
        abort(404, 'Enlace de descarga inválido o expirado.');
    }

    ini_set('memory_limit', '512M');
    set_time_limit(1000);
    $titleDate = !empty($filters['filter_date']) ? ' ' . $filters['filter_date'] : '';
    $title = $filters['filter_type'] ?? '';

    return Excel::download(
        new GeneralReport($filters, 'REPORTE GENERAL ' . $title . $titleDate),
        'reporte-general-' . ($title ? $title . '-' : '') . 'reporte.xlsx'
    );
  }
}
