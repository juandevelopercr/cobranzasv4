...
class Transaction extends Model implements HasMedia
{
  ...
  public function scopeSearch($query, $value, $filters = [])
  {
    ...
    if (!empty($filters['filter_department_name'])) {
      $query->where('transactions.department_id', '=', $filters['filter_department_name']);
    }
  ...
  public function scopeSearch($query, $value, $filters = [])
  {
    $columns = [
      ...
      DB::raw("
            CASE
                WHEN transactions.proforma_type = 'GASTO' THEN
                    (SELECT distinct(ref.id) FROM transactions ref WHERE ref.transaction_reference_id = transactions.id ORDER BY ref.id DESC  LIMIT 1)
                ELSE
                    (SELECT distinct(ref.id) FROM transactions ref WHERE ref.`key` = transactions.RefNumero ORDER BY ref.id DESC  LIMIT 1)
            END as reference_id
        "),
      'totalHonorarios',
      'totalTimbres',
  ...
  */
  ...
  public function scopeSearch($query, $value, $filters = [])
  {
    ...
    $columns = [
      ...
      "),
      DB::raw("(
          SELECT GROUP_CONCAT(TRIM(CONCAT(COALESCE(cc.codigo, ''), ' ', COALESCE(cc.descrip, ''))) SEPARATOR ', ')
          FROM transactions_commissions tc
          LEFT JOIN centro_costos cc ON tc.centro_costo_id = cc.id
          WHERE tc.transaction_id = transactions.id
      ) as centros_costos_html"),
      DB::raw("(
          SELECT IFNULL(SUM(tp.total_medio_pago), 0)
          FROM transactions_payments tp
          WHERE tp.transaction_id = transactions.id
      ) as payment"),
      DB::raw("(
  ...
  */
  ...
  public function scopeSearch($query, $value, $filters = [])
  {
    $columns = [
      ...
      'totalImpuestomercGravadas',
      'totalImpuestoServExonerados',
      'totalImpuestoMercanciasExoneradas',
      'totalImpuesto',
      'totalServExentos',
      'totalmercExentas',
      'totalOtrosCargos',
    ...
    if (!empty($value)) {
      $query->where(function ($q) use ($value) {
        $q->where('transactions.proforma_no', 'like', "%{$value}%")
          ->orWhere('transactions.consecutivo', 'like', "%{$value}%")
          ->orWhere('transactions.customer_name', 'like', "%{$value}%")
          ->orWhere('transactions.customer_comercial_name', 'like', "%{$value}%")
          ->orWhere('departments.name', 'like', "%{$value}%")
          ->orWhere('banks.name', 'like', "%{$value}%")
          ->orWhere('users.name', 'like', "%{$value}%")
          ->orWhere('business_locations.name', 'like', "%{$value}%")
          ->orWhere('currencies.code', 'like', "%{$value}%")
          ->orWhere('transactions.proforma_type', 'like', "%{$value}%");
      });
    }
    ...
    foreach (
      ...
      if (!empty($filters["filter_{$filter}"])) {
        $query->where("transactions.{$filter}", 'like', '%' . $filters["filter_{$filter}"] . '%');
      }
    }
    ...
    foreach (
      ...
      if (!empty($filters["filter_{$filter}"])) {
        $query->where("transactions.{$column}", $filters["filter_{$filter}"]);
      }
    }

    // Filtro por estado con lógica condicional
    if (!empty($filters['filter_status'])) {
      $status = $filters['filter_status'];
      $query->where(function ($q) use ($status) {
        $q->where(function ($q1) use ($status) {
          $q1->whereIn('document_type', [Transaction::PROFORMA, Transaction::NOTACREDITO, Transaction::NOTADEBITO])
            ->where('proforma_status', $status);
        })->orWhere(function ($q2) use ($status) {
          $q2->whereNotIn('document_type', [Transaction::PROFORMA, Transaction::NOTACREDITO, Transaction::NOTADEBITO])
            ->where('status', $status);
        });
      });
    }
    ...
    foreach (
      ...
      if (!empty($filters["filter_{$filterKey}"])) {
        $range = explode(' to ', $filters["filter_{$filterKey}"]);
        try {
          if (count($range) === 2) {
            $start = Carbon::createFromFormat('d-m-Y', $range[0])->format('Y-m-d');
            $end = Carbon::createFromFormat('d-m-Y', $range[1])->format('Y-m-d');
            $query->whereBetween("transactions.{$column}", [$start, $end]);
          } else {
            $date = Carbon::createFromFormat('d-m-Y', $filters["filter_{$filterKey}"])->format('Y-m-d');
            $query->whereDate("transactions.{$column}", $date);
          }
        } catch (\Exception $e) {
          // Fecha inválida: puedes hacer Log::warning() si lo deseas
        }
      }
    }
    return $query;
