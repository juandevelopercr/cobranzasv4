...
class CasoProceso extends Model
{
  // Nombre de la tabla
  protected $table = 'casos_procesos';
  ...
  public function scopeSearch($query, $value, $filters = [])
  {
    ...
    $query->select($columns)->join('banks', 'casos_procesos.bank_id', '=', 'banks.id');
    ...
    if (!empty($filters['filter_bank'])) {
      $query->where('bank_id', '=', $filters['filter_bank']);
    }
