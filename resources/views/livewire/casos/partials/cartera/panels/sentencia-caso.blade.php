<div class="row g-6">
  ...
  <div class="col-12 col-sm-6 col-md-4 col-lg-3 fv-plugins-icon-container">
    <label class="form-label" for="pfecha_primer_giro">{{ __('Fecha 1er Giro') }}</label>
    <div class="input-group input-group-merge has-validation">
      <span class="input-group-text"><i class="bx bx-calendar"></i></span>
      <input type="text" id="pfecha_primer_giro"
        wire:model="pfecha_primer_giro"
        x-data="datePickerLivewire({ wireEventName: 'dateSelected' })"
        x-init="init($el)"
        wire:ignore
        class="form-control date-picke @error('pfecha_primer_giro') is-invalid @enderror">
    </div>
    @error('pfecha_primer_giro')
    <div class="text-danger mt-1">{{ $message }}</div>
    @enderror
  </div>
