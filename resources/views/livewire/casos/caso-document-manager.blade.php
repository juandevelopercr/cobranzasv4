                    ...
                    <div class="row g-4">
                        ...
                        <div class="col-md-5">
                            <div class="border-2 border-dashed rounded p-4 d-flex flex-column justify-content-center align-items-center text-center cursor-pointer transition-all position-relative"
                                ...
                                <input type="file" wire:model="file_by_collection.{{ $collection }}"
                                    x-ref="fileInput" class="d-none" id="file_upload_{{ $collection }}"
                                    accept=".pdf,.doc,.docx,.xls,.xlsx,.png,.jpg,.jpeg">

                                <!-- Loading State -->
                                <div wire:loading wire:target="file_by_collection.{{ $collection }}"
                                ...
                                <!-- Preview / Instruction -->
                                @php
                                    $currentFile = $file_by_collection[$collection] ?? null;
                                @endphp

                                @if ($currentFile)
                                    @php
                                        $ext = strtolower(
                                            pathinfo($currentFile->getClientOriginalName(), PATHINFO_EXTENSION),
                                        );
                                    @endphp
                                    <div
                                        class="d-flex flex-column align-items-center animate__animated animate__fadeIn">
                                        @if (in_array($ext, ['jpg', 'jpeg', 'png', 'gif']))
                                            <img src="{{ $currentFile->temporaryUrl() }}"
                                                class="img-fluid rounded shadow-sm mb-3"
                                                style="max-height: 120px; object-fit: cover;" alt="Preview">
                                        @else
                                            <i class='bx {{ getFileIconClass($currentFile->getMimeType()) }}'
                                        ...
                                        <span
                                            class="badge bg-label-primary mt-1">{{ number_format($currentFile->getSize() / 1024, 2) }}
                                            KB</span>
                                        <button type="button" class="btn btn-sm btn-label-danger mt-3"
                                            wire:click="$set('file_by_collection.{{ $collection }}', null); $refs.fileInput.value = ''"
                                            @click.stop>
                            ...
                            @error('file_by_collection.' . $collection)
                        ...
                        <div class="col-md-7 d-flex flex-column justify-content-center">
                            <div class="mb-3">
                                ...
                                <div class="input-group input-group-merge">
                                    ...
                                    <input type="text" wire:model="title_by_collection.{{ $collection }}"
                                        class="form-control"
                                        placeholder="{{ __('ej., Contrato Firmado, Factura #123...') }}">
                                </div>
                                @error('title_by_collection.' . $collection)
                                ...
                                <div class="d-flex flex-column gap-2 animate__animated animate__fadeIn">
                                    ...
                                    <div class="d-flex gap-2 justify-content-end">
                                        <button wire:click="updateDocument" class="btn btn-xs btn-primary"
                                            ...
                                            <i class="bx bx-check" wire:loading.remove wire:target="updateDocument"></i>
                                        ...
                                        @if ($this->onlyview == false)
                                            ...
                                            @if ($canedit)
                                                <button class="btn p-0 text-primary"
                                                    ...
                                                    data-bs-toggle="tooltip" title="{{ __('Editar') }}">
